// 通用 Node.js/Edge/Cloudflare/Vercel 入口，支持中文和英文自动切换
export default async function handler(req, res) {
  // 判断环境：Cloudflare Workers/Pages 或 Vercel
  let request, respond;
  if (typeof Request !== "undefined" && req instanceof Request) {
    request = req;
    respond = (body, opts) => new Response(body, opts);
  } else {
    // Vercel环境
    request = {
      url: req.url.startsWith("http") ? req.url : `https://${req.headers.host}${req.url}`,
  ...
  if (url.pathname === "/") {
    return respond(generateHTML(userLang), {
      headers: {
  ...
  return respond(notFoundText, { status: 404 });
}

// HTML生成部分（把CSS和JS原样复制即可）
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function convertConfig() {
        try {
          ...
          // 端口范围显示
          portRangeDiv.textContent = numProxies > 0 ? \`Port Range: \${port} ~ \${endPort}\` : '';
          const newYAML = {
            'allow-lan': true,
